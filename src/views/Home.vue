    ...
    <div class="file_setting">
      <div class="file_box_set">
         <div class="box_check">
          ...
          <div>
             <el-checkbox :value="autoDownLoad" @change="onClickAutoDownLoad">新任务自动下载</el-checkbox>
             <el-checkbox v-model="outWindow">任务完成后关机</el-checkbox>
...
export default {
  ...
  data(){
    return{
      ...
      typeFile:['原片','原图','初修片','初修片','选修片','选修片','精修片','精修片','设计片','设计片','微视频','微视频','已选片'],
   ...
   methods: {
     ...
     async getDownHttpSelect(tableData){
        let {fileImg,filePathName,httpPath,OrderNumber,CustomerName,downType,fileID,SubOrderNumber,jsonCreateTime,CustomerMobile,FileNameContainsProductName,make,idImg,makeJson} = tableData;
     ...
     async getDownHttp(tableData){
        let {fileImg,filePathName,httpPath,OrderNumber,CustomerName,downType,fileID} = tableData;
        let url = httpPath +'/fserver/IDownloadFile'

        let fileTitle =`${OrderNumber}(${CustomerName}_${this.typeFile[downType-1]})`
        let jsonParse ={
          fileImg,filePathName,url,fileTitle,fileID
        }
